...
def worker(proc_num, queue, out_dir, in_dir, use_words, count_dir, sample=1e-5):
    while True:
        try:
...
def run_parallel(num_procs, out_dir, in_dir, years, wordlist, count_dir, sample):
    queue = Queue()
    for year in years:
        queue.put(year)
    procs = [Process(target=worker, args=[i, queue, out_dir, in_dir, wordlist, count_dir, sample]) for i in range(num_procs)]
    for p in procs:
        p.start()
    for p in procs:
        p.join()
...
if __name__ == '__main__':
    ...
    run_parallel(args.workers, args.out_dir + "/", args.in_dir + "/", years, wordlist, args.count_dir, args.sample)
